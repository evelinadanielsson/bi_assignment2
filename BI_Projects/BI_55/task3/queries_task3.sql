...
# 6th - needs to be fixed
SELECT Product_Top_Category, Product_Sub_Category, Product_Model, Quantity_Sold
FROM (SELECT ProductTopCategoryName as Product_Top_Category, ProductSubCategoryName as Product_Sub_Category, ProductModelName as Product_Model, sum(OrderQty) Quantity_Sold,
		ROW_NUMBER() OVER (PARTITION BY Product_Top_Category Order by Quantity_Sold DESC) as seqnum
      FROM BI_BikesDW_55.Fact_InternetSales
      INNER JOIN BI_BikesDW_55.Dim_Product on BI_BikesDW_55.Fact_InternetSales.ProductKey = BI_BikesDW_55.Dim_Product.ProductKey)
WHERE seqnum = 3 and Country = "United Kingdom" and ShipMethod = "Cargo International" and (OrderDate between '2020/01/01' and '2020/06/30')
ORDER BY Product_Top_Category, Quantity_Sold;
